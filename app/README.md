...
页面底部提供关于与隐私说明，数据清单见
[`docs/privacy-and-local-data.md`](../docs/privacy-and-local-data.md)。清空操作只删除历史记录与今日一牌两个应用自有键。
...
当前牌环展示洗牌后的完整78张牌背，点击位置决定实际抽牌；多牌阵中已选择位置会被锁定，避免重复抽取。今日一牌继续按日期稳定生成，并直接翻开当天卡牌。微信小程序当前只开放单牌、时间流和凯尔特十字，暂不增加圣三角、恋爱关系五牌阵与二选一牌阵。
